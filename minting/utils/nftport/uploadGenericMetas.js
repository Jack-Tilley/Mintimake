...
if (!fs.existsSync(path.join(`${basePath}/build`, "/ipfsMetas_generic"))) {
  // change this when using generic vs reg
  fs.mkdirSync(path.join(`${basePath}/build`, "ipfsMetas_generic"));
}
...
async function main() {
  ...
  for (const file of files) {
    if (file !== "_metadata.json" && file !== "_ipfsMetas.json") {
      ...
      const uploadedMeta = `${basePath}/build/ipfsMetas_generic/${color_id}.json`;
    ...
    fs.writeFileSync(
      `${basePath}/build/ipfsMetas_generic/_ipfsMetas.json`,
...
const writeResponseMetaData = (_data) => {
  const color_id = _data.attributes.find(
    ({ trait_type }) => trait_type === "color_id"
  ).value;
  fs.writeFileSync(
    `${basePath}/build/ipfsMetas_generic/${color_id}.json`,
